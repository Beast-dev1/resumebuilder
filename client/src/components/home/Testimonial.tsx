...
export default function Testimonial({ testimonials = defaultTestimonials, animationDuration = 25 }: TestimonialProps) {
  ...
  const TestimonialCard = ({ card }: { card: TestimonialCard }) => (
    <div className="p-4 rounded-lg mx-4 shadow hover:shadow-lg transition-all duration-200 w-72 shrink-0">
      ...
      <p className="text-sm pt-4 text-gray-800">{card.text}</p>
    </div>
  );
  ...
  return (
    <section id="testimonials">
      ...
      <Badge text="Testimonials" variant="blue" />
      
      <SectionTitle
        title="What our users say"
        description="Join thousands of satisfied users who have transformed their careers with our resume builder."
      />

      <div className="marquee-row w-full mx-auto max-w-5xl overflow-hidden relative">
        <div className="absolute left-0 top-0 h-full w-20 z-10 pointer-events-none bg-gradient-to-r from-white to-transparent"></div>
        <div className="marquee-inner flex transform-gpu min-w-[200%] pt-10 pb-5">
          {doubledTestimonials.map((card, index) => (
